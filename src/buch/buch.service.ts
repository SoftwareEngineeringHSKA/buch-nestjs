...
import {BuchInvalid, IsbnExists, TitelExists, BuchServiceError, BuchNotExists, VersionInvalid, VersionOutdated} from './errors';
...
export class BuchService {
  ...
  async create(
    ...
    const id: string =
        saved.id.toString();  // eslint-disable-line
                              // @typescript-eslint/no-unsafe-assignment
    logger.debug(`create: id=${id}`);
  ...
  async find(query?: FilterQuery<BuchDocument>|undefined) {
    ...
    if (titel && typeof titel === 'string') {
      ...
      dbQuery.titel =
          ...
          new RegExp(
              titel,
              'iu')  // eslint-disable-line security/detect-non-literal-regexp,
  ...
  async update(
      ...
      id: string,
      version: string,
      ): Promise<|BuchInvalid|BuchNotExists|TitelExists|VersionInvalid|
    ...
    logger.debug(`update: versionUpdated=${versionUpdated}`);
  ...
  async delete(idStr: string) {
    logger.debug(`delete: idStr=${idStr}`);
    if (!ObjectID.isValid(idStr)) {
      logger.debug('delete: Keine gueltige ObjectID');
      return false;
    }

    // Das Buch zur gegebenen ID asynchron loeschen
    const deleted = await this.buchModel.findByIdAndDelete(new ObjectID(idStr))
                        .lean<Buch|null>();
  ...
  private async validateCreate(buch: Buch) {
    ...
    if (msg.length > 0) {
      logger.debug(
          `#validateCreate: Validation Message: ${msg}`,
      );
  ...
  private async validateUpdate(buch: Buch, id: string, versionStr: string) {
    ...
    logger.debug(
        `#validateUpdate: buch=${buch}, version=${version}`,
    );
    ...
    logger.debug('#validateUpdate: ok');
    return undefined;
  }
  ...
  private validateVersion(versionStr: string|undefined) {
    if (versionStr === undefined) {
      ...
      logger.debug(
          `#validateVersion: VersionInvalid=${error}`,
      );
      return error;
    }
  ...
  private async checkTitelExists(buch: Buch) {
    ...
    if (result !== null) {
      const id = (result.id as ObjectID)
                     .toString();  // eslint-disable-line
                                   // @typescript-eslint/no-base-to-string
      logger.debug(`#checkTitelExists: id=${id}`);
      return new TitelExists(titel, id);
    }

    logger.debug('#checkTitelExists: ok');
    return undefined;
  }
  ...
  private async checkIdAndVersion(id: string, version: number) {
    ...
    if (buchDb === null) {
      ...
      logger.debug(
          `#checkIdAndVersion: BuchNotExists=${result}`,
      );
      return result;
    }
    ...
    if (version < versionDb) {
      ...
      logger.debug(
          `#checkIdAndVersion: VersionOutdated=${result}`,
      );
      return result;
    }
