...
const checkChars = (chars: string[]) => {
  ...
  if (chars.length === 9) {
    ...
    check = 11 -
        (sum % 11);  // eslint-disable-line @typescript-eslint/no-extra-parens
    if (check === 10) {
      check = 'X';
    } else if (check === 11) {
      check = '0';
    ...
    check = 10 -
        (sum % 10);  // eslint-disable-line @typescript-eslint/no-extra-parens
    if (check === 10) {
      check = '0';
    }
  }
...
const validateISBN: FormatValidator<string> = (subject: string) => {
  ...
  if (regex.test(subject)) {
    ...
    const chars = subject.replace(/[ -]|^ISBN(?:-1[03])?:?/gu, '').split('');
...
export const validateBuch = (buch: Buch) => {
  ...
  const messages =
      errors.map((error) => error.message).filter((msg) => msg !== undefined);
  logger.debug(
      `validateBuch: errors=${
          safeStringify(
              errors,
